...
const DoctorCardIC = ({ name, speciality, experience, ratings, profilePic }) => {
  ...
  const [appointmentBooked, setAppointmentBooked] = useState(false); // State to track appointment booking
  ...
  const handleCancel = (appointmentId) => {
    ...
    setAppointmentBooked(false); // Cancel appointment, update state
  };
  ...
  const handleFormSubmit = (appointmentData) => {
    ...
    setAppointments(updatedAppointments);
    setAppointmentBooked(true); // Book appointment, update state

    // Store appointment data in localStorage
    localStorage.setItem('appointmentData', JSON.stringify(newAppointment));
    console.log("appointement booked :", JSON.stringify(newAppointment));
    setShowModal(false);
  };
